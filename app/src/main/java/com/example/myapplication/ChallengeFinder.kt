...
class ChallengeFinder : Fragment() {
    ...
    private fun addDataToList() {
        mList.apply {
            ...
            add(ChallengeData("CS2", "Your team’s counting on you! Train hard, aim sharp, and dominate the challenge — this is your moment!", 1314, "Win a match. No armor. Shotgun only.", ResourceType.TCHL, "1 bicycle kicks", 1, arrayOf(false, true, true, false).toBooleanArray(),  4))
            add(ChallengeData("Valorant", "Ready to take control of the map and your fitness? Step up, agent — this challenge is yours to win!", 1223, "Knife at least 3 people next round.", ResourceType.RCHL, "50 calf-raises", 2, arrayOf(false, true, true, false).toBooleanArray(), 40))
            add(ChallengeData("Rocket League", "Drift. Dodge. Dominate. Pull yourself up to victory if you have to.", 8845, "Get at least 2 aerial goals next match.", ResourceType.RCHL, "10 pull ups", 3, arrayOf(true, false, true, false).toBooleanArray(), 11))
            add(ChallengeData("NBA 2K", "Train like a legend. Play like a champ.", 7946, "Win 3 consecutive games.", ResourceType.RCHL, "7-7-7-7 bicep curls", 3,  arrayOf(true, false, false, false).toBooleanArray(), 35))
            add(ChallengeData("MLB The Show", "Make it a home run or Coach will be angry!", 7745, "You must make at least 7 home-runs on your next game.", ResourceType.TCHL, "1 minute planking", 1,  arrayOf(false, false, true, false).toBooleanArray(), 5))
            add(ChallengeData("Overwatch", "Show no Mercy >:)", 1469, "Have at least 21 eliminations in a game.", ResourceType.TCHL, "30 flutter kicks", 1, arrayOf(false, true, true, false).toBooleanArray(), 15))
